...
# The renamed reference, FASTA index, and chromosome list are generated by
# alignment.smk. During the first invocation they may not exist yet, so do not
# read them eagerly while Snakemake is parsing the workflow.
_reference_metadata_ready = Path(CHROM_LIST).is_file() and Path(REF_FAI).is_file()

if _reference_metadata_ready:
    with open(CHROM_LIST) as handle:
        CHROMOSOMES = [line.strip() for line in handle if line.strip()]
    ...
    CHROMOSOMES = []
    chrom_length = pd.Series(dtype="int64")
...
RAW_BCF_TARGETS = [
    f"{VC_ROOT}/bcf/raw/all_sites.{chrom}.{chunk}.{ext}"
    for chrom in CHROMOSOMES
    for chunk in get_chunks(chrom)
...
FINAL_VCF_TARGETS = (
    ...
    + expand(
        f"{VC_ROOT}/vcf/biallelic_snps.{IND_FILTER_ID}.{SITE_FILTER_ID}.{{chrom}}.vcf.gz.tbi",
        chrom=CHROMOSOMES,
    )
)
...
rule call_all_sites_bcf:
    ...
    output:
        ...
        csi = f"{VC_ROOT}/bcf/raw/all_sites.{{chrom}}.{{chunk}}.bcf.csi",
...
rule genotype_filter_all_sites:
    ...
    output:
        ...
        csi = f"{VC_ROOT}/bcf/genotype_filtered/all_sites.{{ind_filter_id}}.{{chrom}}.{{chunk}}.bcf.csi",
...
rule add_site_filters:
    ...
    output:
        ...
        csi = f"{VC_ROOT}/bcf/site_filtered/all_sites.{{ind_filter_id}}.{{site_filter_id}}.{{chrom}}.{{chunk}}.bcf.csi"
    params:
...
rule make_chrom_biallelic_snps:
    input:
        bcf = f"{VC_ROOT}/bcf/final/variants.{{ind_filter_id}}.{{site_filter_id}}.{{chrom}}.bcf"
    output:
        bcf = f"{VC_ROOT}/bcf/final/biallelic_snps.{{ind_filter_id}}.{{site_filter_id}}.{{chrom}}.bcf",
        csi = f"{VC_ROOT}/bcf/final/biallelic_snps.{{ind_filter_id}}.{{site_filter_id}}.{{chrom}}.bcf.csi"
    shell:
        r"""
        bcftools view \
          --max-alleles 2 \
...
rule clean_intermediates:
    ...
    shell:
        ...
        rm -rf {REPORT_ROOT}/per_chunk
        rm -f {VC_ROOT}/coverage/total_coverage_*.txt
        """
...
rule clean_snakemake:
    message:
        "Removing .snakemake working directory."
    shell:
        r"""
        rm -rf .snakemake
        """
